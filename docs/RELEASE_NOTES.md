## Smart Water Lab v1.0 — Course Submission

**Mahmudul Hasan (4125999049)** · Xi'an Jiaotong University · 2026

### Included PDFs

- AI-Augmented Engineering Case Study for Smart Water Systems  
- Experiment 1 — Rainfall Monitoring & Alert  
- Experiment 2 — SCS-CN Runoff  
- Experiment 3 — Reservoir Optimization  
- Experiment 4 — Flood Inundation  

### Engineering summary

| Metric | Value |
|--------|------:|
| Automated tests (this repository) | **33** |
| Additional experiment tests (local dev folders) | **88** |
| Validation CLIs | 4 |
| AI corrections documented | 9 |
| Monte Carlo scenarios | 100 |

Integrated Smart Water Decision Support Pipeline with Swiss Cheese verification and threats-to-validity analysis.

See repository README for capstone setup and figure gallery.
