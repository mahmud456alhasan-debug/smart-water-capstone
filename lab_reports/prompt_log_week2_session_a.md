...
The result is far too small (expected ~43.6 mm), which confirms the script is wrong. Main cause: `S = 25400 * CN - 254` makes S enormous (~2.16×10⁶ mm), so the denominator `(P + S)` dominates and Q ≈ 0.
...
|-----|----------|------------|-------|
| S uses `*` not `/` | Yes | Line 9: `25400 * CN` inflates S to ~2.16×10⁶ mm | Yes |
